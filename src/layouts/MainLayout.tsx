    ...
    <div className="flex flex-col h-full bg-blue-950 border-r border-blue-900/50">
      ...
      <div className="px-5 py-5 border-b border-blue-900/50">
        <div className="flex items-center gap-3">
          <Logo />
          <div>
            <div className="text-white font-bold text-sm leading-none tracking-wide">
              Ticketing Genie
            </div>
...
export const MainLayout: React.FC<MainLayoutProps> = ({
  ...
  // ── SSE — increment/decrement badge in real time ──────────────────────────
  const connectSSE = useCallback(() => {
    if (!accessToken) return;

    // Use the existing notifications SSE stream
    ...
    es.addEventListener('read_receipt', () => {
      // Notification marked read — decrement badge, floor at 0
  ...
  useEffect(() => {
    ...
    // Poll every 60s as a fallback in case SSE misses an event
  ...
  // When user navigates to /notifications, reset badge optimistically
  const location = useLocation();
  useEffect(() => {
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-14 border-b border-blue-100 bg-white flex items-center px-4 flex-shrink-0 gap-3 shadow-sm">
          ...
          {pageTitle && (
            <h1 className="text-slate-700 font-semibold text-sm truncate">
              {pageTitle}
            </h1>
          )}

          {/* Notification bell in top bar — mobile-friendly quick access */}
