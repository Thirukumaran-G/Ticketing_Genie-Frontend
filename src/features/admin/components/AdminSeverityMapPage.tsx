// // src/features/admin/components/AdminSeverityMapPage.tsx
// // GET /admin/severity-priority-map, POST (upsert by severity+tier_id), DELETE /{id}
...
// import { Button, Modal, PageLoader } from '../../../components/ui/index';
// import { adminTicketService } from '../services/adminTicketService';
// import { adminNav } from './adminNav';
// import { SeverityPriorityMapResponse } from '../../../types';
// import { SEVERITIES, PRIORITIES } from '../../../config';
...
// const SEV_COLORS: Record<string, string> = { critical: 'text-red-400', high: 'text-orange-400', medium: 'text-yellow-400', low: 'text-blue-400' };
// const PRI_COLORS: Record<string, string> = { P0: 'text-red-400', P1: 'text-orange-400', P2: 'text-yellow-400', P3: 'text-blue-400' };
...
//   ]).then(([m, t]) => { setMaps(m); setTiers(t); }).finally(() => setLoading(false));
...
//             <p className="text-zinc-500 text-sm mt-1">Maps customer-reported severity + tier to a derived ticket priority</p>
//           </div>
...
//               <div className="w-28"><span className={`text-sm font-bold capitalize ${SEV_COLORS[m.severity] ?? 'text-white'}`}>{m.severity}</span></div>
//               <div className="flex-1 text-sm text-zinc-300">{tierName(m.tier_id)}</div>
//               <div className="w-24"><span className={`text-sm font-bold ${PRI_COLORS[m.derived_priority] ?? 'text-white'}`}>{m.derived_priority}</span></div>
//               <div className="w-20"><Button size="sm" variant="danger" onClick={() => onDelete(m.id)}>Delete</Button></div>
//             </div>
//           ))}
//         </div>
//       </div>
...
//           <div>
//             <label className="block text-xs font-semibold text-zinc-400 mb-1.5 uppercase tracking-widest">Severity</label>
//             <select className="w-full bg-zinc-900 border border-zinc-700 text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-white" {...register('severity')}>
...
//           </div>
//           <div>
//             <label className="block text-xs font-semibold text-zinc-400 mb-1.5 uppercase tracking-widest">Derived Priority</label>
//             <select className="w-full bg-zinc-900 border border-zinc-700 text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-white" {...register('derived_priority')}>
//               <option value="">Select priority…</option>
//               {PRIORITIES.map(p => <option key={p} value={p}>{p}</option>)}
//             </select>
//             {errors.derived_priority && <p className="text-xs text-red-400 mt-1">{errors.derived_priority.message}</p>}
//           </div>
