...
const SEV_VARIANT: Record<string, any> = { critical: 'critical', high: 'high', medium: 'medium', low: 'low' };
...
const Sel: React.FC<React.SelectHTMLAttributes<HTMLSelectElement> & { label: string; error?: string; disabled?: boolean }> = ({
  label, error, children, disabled, ...props
}) => (
  <div>
    <label className="block text-xs font-semibold text-zinc-400 mb-1.5 uppercase tracking-widest">{label}</label>
    <select disabled={disabled} className="w-full bg-zinc-900 border border-zinc-700 text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-white disabled:opacity-50" {...props}>
      {children}
    </select>
...
export const AdminProductConfigPage: React.FC = () => {
  ...
  const load = async () => {
    ...
    setConfigs(c); setProducts(p); setLoading(false);
  };
  useEffect(() => { load(); }, []);
    ...
    <MainLayout navItems={adminNav} pageTitle="Product Config">
        ...
        <div className="bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden">
          ...
          {loading ? <PageLoader /> : configs.length === 0 ? (
            ...
            <div key={c.id} className="flex items-center gap-4 px-6 py-4 border-b border-zinc-900 hover:bg-zinc-900/30">
              ...
              <div className="w-32">
                {c.min_severity
                  ? <Badge variant={SEV_VARIANT[c.min_severity] ?? 'default'}>{c.min_severity}</Badge>
      ...
      <Modal open={showModal} onClose={onClose} title={editing ? 'Edit Product Config' : 'New Product Config'}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
          <Sel label="Product" disabled={!!editing} error={errors.product_id?.message} {...register('product_id')}>
          ...
          <div className="flex items-center gap-3 bg-zinc-900 rounded-lg p-4 border border-zinc-800">
            ...
            <label htmlFor="escalate" className="text-sm text-zinc-300 cursor-pointer">
              Auto-escalate all tickets for this product
            </label>
          </div>
