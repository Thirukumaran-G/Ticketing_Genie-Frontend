// src/features/admin/components/AdminKeywordsPage.tsx
...
export const AdminKeywordsPage: React.FC = () => {
  ...
  const load = () => adminTicketService.listKeywordRules().then(setRules).finally(() => setLoading(false));
  useEffect(() => { load(); }, []);
  ...
  const onSubmit = async (d: Form) => {
    try {
      setSubmitting(true);
      if (editing) { await adminTicketService.updateKeywordRule(editing.id, d); toast.success('Updated'); }
      else { await adminTicketService.createKeywordRule(d); toast.success('Created'); }
      onClose(); load();
        ...
        <div className="bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden">
          ...
          {loading ? <PageLoader /> : rules.length === 0 ? (
            ...
            <div key={r.id} className="flex items-center gap-4 px-6 py-4 border-b border-zinc-900 hover:bg-zinc-900/30">
              ...
              <div className="w-28"><Badge variant={SEV_VARIANT[r.severity] ?? 'default'}>{r.severity}</Badge></div>
              <div className="w-20"><Badge variant={r.is_active ? 'success' : 'default'}>{r.is_active ? 'Yes' : 'No'}</Badge></div>
              <div className="w-28 flex gap-2">
                <Button size="sm" variant="secondary" onClick={() => openEdit(r)}>Edit</Button>
                <Button size="sm" variant="danger" onClick={() => onDelete(r.id)}>Off</Button>
              </div>
            </div>
          ))}
        </div>
