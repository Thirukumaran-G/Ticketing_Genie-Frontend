...
export const SeverityDot: React.FC<{ severity: string }> = ({ severity }) => (
  <span className={clsx('inline-block w-2 h-2 rounded-full flex-shrink-0', SEV_DOT[severity] ?? 'bg-slate-400')} />
);

export const PriorityLabel: React.FC<{ priority: string }> = ({ priority }) => (
  <span className={clsx('font-bold text-xs', PRI_CLS[priority] ?? 'text-slate-500')}>{priority}</span>
);
