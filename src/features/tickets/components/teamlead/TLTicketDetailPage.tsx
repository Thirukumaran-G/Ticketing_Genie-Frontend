...
const ThreadBubble: React.FC<{ entry: ThreadEntry; ticketId: string }> = ({ entry, ticketId }) => {
  if (entry.kind === 'attachment') {
    ...
    const url = `${ticketsService.getTLTicketThread ? '' : ''}`;
    return (
      <div className="flex gap-2.5">
        <div className="w-7 h-7 rounded-full bg-zinc-700 flex items-center justify-center text-[10px] font-bold flex-shrink-0 mt-0.5 text-zinc-300">
          {getInitials('Customer')}
        </div>
        <div className="flex flex-col gap-1 items-start">
          <div className="flex items-center gap-3 px-3.5 py-2.5 rounded-2xl rounded-tl-sm bg-zinc-800/80 border border-zinc-700/60 text-zinc-200">
  ...
  return (
    <div className={clsx('flex gap-2.5', isRight ? 'flex-row-reverse' : 'flex-row')}>
      <div className={clsx('w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 mt-0.5', avatarColor)}>
        {avatarLabel}
      </div>
    ...
    <MainLayout navItems={tlNav} pageTitle={t.ticket_number}>
      <div className="flex flex-col h-[calc(100vh-56px)]">
        ...
        <div className="px-5 pt-4 pb-3 flex-shrink-0">
          <Link to="/tickets/team/all" className="inline-flex items-center gap-2 text-zinc-500 hover:text-white text-sm transition-colors">
        ...
        <div className="flex flex-1 gap-4 px-5 pb-5 min-h-0 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col min-w-0 bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden">
            ...
            <div className="px-5 py-3.5 border-b border-zinc-800 flex-shrink-0">
              ...
              <div className="flex items-center gap-4 mt-2">
                ...
                <div className="flex items-center gap-1.5 text-[11px] text-zinc-500">
                  <span className="w-2 h-2 rounded-full bg-zinc-500 inline-block" /> Customer
                </div>
                <div className="flex items-center gap-1.5 text-[11px] text-zinc-500">
            ...
            <div className="flex-shrink-0 px-4 pb-4 pt-3 border-t border-zinc-900">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-semibold text-purple-400 uppercase tracking-widest">Internal Note (Team Lead only)</span>
              </div>
              <div className="flex items-start gap-2 bg-purple-950/20 border border-purple-900/40 focus-within:border-purple-700/60 rounded-xl px-4 py-3 transition-colors">
                ...
                <button
                  ...
                  className="flex-shrink-0 p-1.5 rounded-lg bg-purple-700 hover:bg-purple-600 text-white disabled:opacity-40 disabled:cursor-not-allowed transition-all mt-0.5"
                >
                  {sendingNote
              ...
              <p className="text-[11px] text-zinc-700 mt-1.5 px-1">Enter to save · never visible to customer</p>
            </div>
          </div>
          ...
          <div className="w-80 flex-shrink-0 flex flex-col gap-3 overflow-y-auto">
            ...
            <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-5">
              ...
              <h2 className="text-white font-semibold text-sm leading-snug mb-3">{t.title ?? '(No title)'}</h2>
            ...
            {(t.sla_response_due || t.sla_resolve_due) && (
              <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-5">
                <p className="text-xs font-semibold text-zinc-500 uppercase tracking-widest mb-3">SLA</p>
                {t.sla_response_due && (
            ...
            <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-5">
              ...
              <p className="text-zinc-300 text-sm leading-relaxed whitespace-pre-wrap">{t.description ?? 'No description provided.'}</p>
            </div>
